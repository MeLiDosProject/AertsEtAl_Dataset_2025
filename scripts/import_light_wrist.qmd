...
```{r}
data <- import$ActLumus(files, tz = tzs[site], auto.id = pattern, dst_adjustment = TRUE)
```
...
Only two days remains with gap/irregular data. One is due to the daylight savings time jump, the other due to an irregular start.
...
data |> 
  ...
  group_modify(
    \(dat,key) if(key$Id == "THUAS_S014") {
      dat |>  aggregate_Datetime()
    } else dat
  )

data_cleaned |> count_difftime() |> filter(difftime == 0) |> nrow() != 0 #no duplicates
