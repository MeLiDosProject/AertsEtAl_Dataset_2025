...
#import files
data_paper <- 
  read_csv2(files, show_col_types = FALSE, id = "record_id") |> 
  mutate(record_id = record_id |> str_extract(paste0(site, "_S.{3}"))) |> 
  mutate(start = 
           timestamp |> 
           parse_date_time(tz = tzs[[site]], orders = c("dmyHM")) |> 
...
data <- 
data |>
  mutate(activity = paste0(activity, activity_desc |> str_extract_all("also [:digit:]")),
         activity_desc = str_remove_all(activity_desc, "also [:digit:]"))
```
...
#   filter(!is.na(activity_desc) & activity_desc != "")
# 
# data_red <- data_red1 |>
...
```{r}
data <- data |> rename(Id = record_id) |> relocate(start, .before = end)
